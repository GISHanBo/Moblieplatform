...
function startDrawDevice() {
    ...
    events.device_map_click=function(e){
        //设置图标样式
    ...
    map.on("click", events.device_map_click);

    //右键停止绘制设备，移动端长按
    events.device_map_context=function(e){
        stopDrawDevice()
    };
    map.on("contextmenu", events.device_map_context);
}
...
function stopDrawDevice() {
    //移除点击监听
    map.off('click',events.device_map_click);
    map.off('contextmenu',events.device_map_context);
    delete events.device_map_click;
    delete events.device_map_context;
